...
function RestaurantListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RestaurantInterface[]>(
    ...
    () =>
      getRestaurants({
        relations: [
          'user_restaurant_user_idTouser',
          'menu.count',
          'reservation.count',
          'user_user_restaurant_idTorestaurant.count',
        ],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_restaurant_user_idTouser</Th>
                  )}
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_user_restaurant_idTorestaurant</Th>
                  )}
                  <Th>Actions</Th>
                </Tr>
              </Thead>
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_restaurant_user_idTouser?.id}`}>
                          {record.user_restaurant_user_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.user_user_restaurant_idTorestaurant}</Td>
                    )}
                    <Td>
