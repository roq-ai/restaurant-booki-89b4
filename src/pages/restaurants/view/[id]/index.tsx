  ...
  const { data, error, isLoading, mutate } = useSWR<RestaurantInterface>(
    ...
    () =>
      getRestaurantById(id, {
        relations: ['user_restaurant_user_idTouser', 'menu', 'reservation'],
      }),
  );
            ...
            <Box>
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Restaurant User Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_restaurant_user_idTouser?.id}`}>
                      {data?.user_restaurant_user_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
